...
b_file_open		equ 0x00000000001000D0	; Open a file for read/write access. IN: RSI = File name. OUT: RAX = File handle ID
b_file_close		equ 0x00000000001000E0	; Close a file. IN: RAX = File handle ID
b_file_read		equ 0x00000000001000F0	; Read a file from disk into memory. IN: RAX = File handle ID, RDI = Memory location where file will be loaded to, RCX = Number of bytes to read. OUT: Carry is set if the file was not found or an error occured
b_file_write		equ 0x0000000000100100	; Write memory to a file on disk. IN: RAX = File handle ID, RSI = Memory location of data to be written, RCX = Number of bytes to write. OUT: Carry is set if an error occured
b_file_seek		equ 0x0000000000100110	; Generate a list of files on disk. IN: RDI = Location to store list. OUT: RDI = pointer to end of list
b_file_query		equ 0x0000000000100120	; Query the existence of a file. IN: RSI = location of filename. OUT: RCX = Size in bytes, Carry set if file not found
