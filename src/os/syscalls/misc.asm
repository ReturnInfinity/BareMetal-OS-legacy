...
; -----------------------------------------------------------------------------
; os_get_time_data -- 
; IN:	RAX = Pointer to structure to be filled (needs 36 bytes)
os_get_time_data:
	push rdi
	push rbx
	push rax
...
os_get_time_data_wait:
	mov al, 10
	out 0x70, al
	in al, 0x71
	test al, 0x80			; Is there an update in progress?
	jne os_get_time_data_wait	; If so then try again

	mov al, 0x00			; Seconds after the minute (0-60)
	out 0x70, al
	in al, 0x71
	stosd
	mov al, 0x02			; Minutes after the hour (0-59)
	out 0x70, al
	in al, 0x71
	stosd
	mov al, 0x04			; Hours since midnight (0-23)
	out 0x70, al
	in al, 0x71
	stosd
	mov al, 0x07			; Day of the month (1-31)
	out 0x70, al
	in al, 0x71
	stosd
	mov al, 0x08			; Months since January (0-11)
	out 0x70, al
	in al, 0x71
	stosd

	mov al, 0x32			; Years since 1900
	out 0x70, al			; Century
	in al, 0x71
	mov bl, 100			
	mul bl
	...
	xor eax, eax
	mov al, 0x06			; Day number of the Week (0-6)
	out 0x70, al
	in al, 0x71
	sub al, 1			; RTC returns 1-7
	stosd

	xor eax, eax			; Days since January 1
	stosd

	mov al, 0x0B			; Daylight Savings Enable
	out 0x70, al
	in al, 0x71
	and al, 00000001b
	stosd
